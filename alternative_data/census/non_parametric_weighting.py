...
if __name__ == "__main__":
    ...
    owned_data['price_to_rent'] =  owned_data['VALUE'] / (12.0 * owned_data['RENT'])
    for i in owned_data.year.unique():
        if i >= 1995:
            sns.distplot(owned_data.loc[owned_data['year']==i,'price_to_rent' ], kde = True, hist = False,kde_kws={'clip': (0.0, 30.0)}, label = i)

    median = {}
    for i in owned_data['year'].unique():
        median[i] = owned_data.loc[owned_data['year']==i,'price_to_rent'].median()
